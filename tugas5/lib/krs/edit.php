...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE krs SET mhs_npm='$mhs_npm', mk_kodemk='$mk_kodemk' WHERE id='$id'";

    if (mysqli_query($conn, $query)) {
        header("Location: index.php");
        exit();
    } else {
        echo "Terjadi kesalahan saat mengupdate data KRS: " . mysqli_error($conn);
    }
}
?>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="mhs_npm" name="mhs_npm">
                    <?php foreach ($data_mahasiswa as $mahasiswa) : ?>
                        <option value="<?php echo $mahasiswa['npm']; ?>" <?php if ($mahasiswa['npm'] === $data['mhs_npm']) echo 'selected'; ?>>
                            <?php echo $mahasiswa['npm']; ?> - <?php echo $mahasiswa['nama']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" id="mk_kodemk" name="mk_kodemk">
                    <?php foreach ($data_matakuliah as $matakuliah) : ?>
                        <option value="<?php echo $matakuliah['kodemk']; ?>" <?php if ($matakuliah['kodemk'] === $data['mk_kodemk']) echo 'selected'; ?>>
                            <?php echo $matakuliah['kodemk']; ?> - <?php echo $matakuliah['nama']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
